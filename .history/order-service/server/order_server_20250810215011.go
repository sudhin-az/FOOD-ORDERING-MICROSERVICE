...
func NewOrderServer(repo *db.OrderRepository) *OrderServer {
...
// Calculate total amount (you might want to fetch actual prices from menu service)
	...
	order, err := s.repo.PlaceOrder(ctx, req.UserId, req.ItemIds, qu, req.DeliveryAddress, totalAmount)
	if err != nil {
...
func (s *OrderServer) GetOrdersByUser(ctx context.Context, req *pb.UserRequest) (*pb.OrderList, error) {
	if req.UserId == "" {
		return nil, fmt.Errorf("user_id is required")
	}
...
func (s *OrderServer) GetAllOrders(ctx context.Context, req *pb.Empty) (*pb.OrderList, error) {
