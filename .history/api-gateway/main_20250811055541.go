...
func main() {
	...
	r.GET("/orders/:user_id", func(ctx *gin.Context) {
		...
		orders, err := orderClient.GetOrdersByUser(context.Background(), req)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	r.GET("/order/:order_id", func(ctx *gin.Context) {
		...
		req := &orderpb.OrderRequest{OrderId: orderID}

		order, err := orderClient.GetOrderById(context.Background(), &req)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	r.PUT("/order/status", func(ctx *gin.Context) {
	...
	r.GET("/orders", func(ctx *gin.Context) {
		orders, err := orderClient.GetAllOrders(context.Background(), &orderpb.Empty{})
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
